// generated with ast extension for cup
// version 0.8
// 18/7/2022 12:23:36


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Mulop Mulop) { }
    public void visit(RecordDecList RecordDecList) { }
    public void visit(ConstructorDecl ConstructorDecl) { }
    public void visit(Relop Relop) { }
    public void visit(Var Var) { }
    public void visit(Else Else) { }
    public void visit(StatementList StatementList) { }
    public void visit(MethodDecList MethodDecList) { }
    public void visit(Addop Addop) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(VarList VarList) { }
    public void visit(ConstList ConstList) { }
    public void visit(Designator Designator) { }
    public void visit(Term Term) { }
    public void visit(Condition Condition) { }
    public void visit(MethodPart MethodPart) { }
    public void visit(Printable Printable) { }
    public void visit(Statements Statements) { }
    public void visit(Label Label) { }
    public void visit(DesignatorName DesignatorName) { }
    public void visit(VarDecList VarDecList) { }
    public void visit(OptionalVarDecList OptionalVarDecList) { }
    public void visit(Expr Expr) { }
    public void visit(ActPars ActPars) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Const Const) { }
    public void visit(Operations Operations) { }
    public void visit(FormParamList FormParamList) { }
    public void visit(Statement Statement) { }
    public void visit(Expression Expression) { }
    public void visit(Type Type) { }
    public void visit(Negative Negative) { }
    public void visit(BaseExp BaseExp) { }
    public void visit(ClassDecList ClassDecList) { }
    public void visit(CondFact CondFact) { }
    public void visit(Declaration Declaration) { }
    public void visit(Param Param) { }
    public void visit(SingleStatement SingleStatement) { }
    public void visit(Field Field) { }
    public void visit(FieldList FieldList) { }
    public void visit(ConstDecList ConstDecList) { }
    public void visit(FormPars FormPars) { }
    public void visit(Mod Mod) { visit(); }
    public void visit(Div Div) { visit(); }
    public void visit(Mul Mul) { visit(); }
    public void visit(NoNegativ NoNegativ) { visit(); }
    public void visit(NegativNum NegativNum) { visit(); }
    public void visit(Minus Minus) { visit(); }
    public void visit(Plus Plus) { visit(); }
    public void visit(LowerEqual LowerEqual) { visit(); }
    public void visit(Lower Lower) { visit(); }
    public void visit(GreaterEqual GreaterEqual) { visit(); }
    public void visit(Greater Greater) { visit(); }
    public void visit(NoEqual NoEqual) { visit(); }
    public void visit(Equal Equal) { visit(); }
    public void visit(VoidType VoidType) { visit(); }
    public void visit(IdentType IdentType) { visit(); }
    public void visit(Labels Labels) { visit(); }
    public void visit(FactorExpr FactorExpr) { visit(); }
    public void visit(New New) { visit(); }
    public void visit(NewExpr NewExpr) { visit(); }
    public void visit(BooleanConst BooleanConst) { visit(); }
    public void visit(CharacterConst CharacterConst) { visit(); }
    public void visit(NumberConst NumberConst) { visit(); }
    public void visit(VarDesig VarDesig) { visit(); }
    public void visit(FuncCall FuncCall) { visit(); }
    public void visit(SingleFactor SingleFactor) { visit(); }
    public void visit(ListFactor ListFactor) { visit(); }
    public void visit(SingleTerm SingleTerm) { visit(); }
    public void visit(Terms Terms) { visit(); }
    public void visit(TermExpr TermExpr) { visit(); }
    public void visit(AddExpr AddExpr) { visit(); }
    public void visit(Express Express) { visit(); }
    public void visit(MinusExpress MinusExpress) { visit(); }
    public void visit(SingleCondFact SingleCondFact) { visit(); }
    public void visit(CondFactor CondFactor) { visit(); }
    public void visit(SingleCondTerm SingleCondTerm) { visit(); }
    public void visit(CondiTerm CondiTerm) { visit(); }
    public void visit(SignleCondition SignleCondition) { visit(); }
    public void visit(Conditions Conditions) { visit(); }
    public void visit(NoActPars NoActPars) { visit(); }
    public void visit(SingleActPars SingleActPars) { visit(); }
    public void visit(ActParams ActParams) { visit(); }
    public void visit(Dec Dec) { visit(); }
    public void visit(Inc Inc) { visit(); }
    public void visit(ActParameters ActParameters) { visit(); }
    public void visit(AssignOperation AssignOperation) { visit(); }
    public void visit(FieldBox FieldBox) { visit(); }
    public void visit(FieldNoBox FieldNoBox) { visit(); }
    public void visit(SingleField SingleField) { visit(); }
    public void visit(Fields Fields) { visit(); }
    public void visit(DesigName DesigName) { visit(); }
    public void visit(SingleDesig SingleDesig) { visit(); }
    public void visit(DesigList DesigList) { visit(); }
    public void visit(DesignatorStatementOp DesignatorStatementOp) { visit(); }
    public void visit(StatementsBraces StatementsBraces) { visit(); }
    public void visit(NoElse NoElse) { visit(); }
    public void visit(ElseStatement ElseStatement) { visit(); }
    public void visit(NoPrintable NoPrintable) { visit(); }
    public void visit(Prints Prints) { visit(); }
    public void visit(SingleStatementDerived1 SingleStatementDerived1) { visit(); }
    public void visit(PrintStatement PrintStatement) { visit(); }
    public void visit(ReadStatement ReadStatement) { visit(); }
    public void visit(VoidReturn VoidReturn) { visit(); }
    public void visit(ReturnExprStatement ReturnExprStatement) { visit(); }
    public void visit(ContinueStatement ContinueStatement) { visit(); }
    public void visit(BreakStatement BreakStatement) { visit(); }
    public void visit(DoWhileStatement DoWhileStatement) { visit(); }
    public void visit(IfStatement IfStatement) { visit(); }
    public void visit(DesignatorStat DesignatorStat) { visit(); }
    public void visit(Stats Stats) { visit(); }
    public void visit(SingleStat SingleStat) { visit(); }
    public void visit(LabelStatement LabelStatement) { visit(); }
    public void visit(NoStatement NoStatement) { visit(); }
    public void visit(ListOfStatements ListOfStatements) { visit(); }
    public void visit(ParamBox ParamBox) { visit(); }
    public void visit(Parameter Parameter) { visit(); }
    public void visit(SingleParam SingleParam) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(NoParams NoParams) { visit(); }
    public void visit(ParamList ParamList) { visit(); }
    public void visit(NoMethodVarDecList NoMethodVarDecList) { visit(); }
    public void visit(HasVarDecList HasVarDecList) { visit(); }
    public void visit(MethodTypeName MethodTypeName) { visit(); }
    public void visit(Method Method) { visit(); }
    public void visit(SingleMethodDeclaration SingleMethodDeclaration) { visit(); }
    public void visit(MethodDeclerations MethodDeclerations) { visit(); }
    public void visit(RecordName RecordName) { visit(); }
    public void visit(RecordDecleration RecordDecleration) { visit(); }
    public void visit(ConstructorDecleration ConstructorDecleration) { visit(); }
    public void visit(NoMethodPart NoMethodPart) { visit(); }
    public void visit(Methods Methods) { visit(); }
    public void visit(ConstructorAndMethod ConstructorAndMethod) { visit(); }
    public void visit(ClassDeclarations ClassDeclarations) { visit(); }
    public void visit(ClassExtendsDeclerations ClassExtendsDeclerations) { visit(); }
    public void visit(VarArray VarArray) { visit(); }
    public void visit(VarNoArray VarNoArray) { visit(); }
    public void visit(SingleVar SingleVar) { visit(); }
    public void visit(VariableList VariableList) { visit(); }
    public void visit(VarDecListDerived1 VarDecListDerived1) { visit(); }
    public void visit(VarDeclerations VarDeclerations) { visit(); }
    public void visit(BoolConstant BoolConstant) { visit(); }
    public void visit(CharConstant CharConstant) { visit(); }
    public void visit(NumConstant NumConstant) { visit(); }
    public void visit(OneConstant OneConstant) { visit(); }
    public void visit(ConstantList ConstantList) { visit(); }
    public void visit(ConstDeclerations ConstDeclerations) { visit(); }
    public void visit(NoDeclaration NoDeclaration) { visit(); }
    public void visit(RecordDecLarationList RecordDecLarationList) { visit(); }
    public void visit(ClassDeclarationList ClassDeclarationList) { visit(); }
    public void visit(VarDeclarationList VarDeclarationList) { visit(); }
    public void visit(ConstDeclarationList ConstDeclarationList) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
